import { PatientDeletedEvent, Subjects, eventLogger, ReplicaDeletedListener } from '@thelarsson/acss-common';
...
export class PatientDeletedListener extends ReplicaDeletedListener<PatientDeletedEvent, Patient> {
  ...
  async onTransaction(data: { id: number; versionKey: number }, row: Patient, transaction: Transaction): Promise<void> {
    ...
    eventLogger.info(`[EVENT] Patient d [OK      ] ${data.id}.${data.versionKey} -> ${row.id}.${row.versionKey - 1}`);
  }

  infoIgnored(data: { id: number; versionKey: number }, row?: any): void {
    eventLogger.info(`[EVENT] Patient d [ACK(IGN)] ${data.id}.${data.versionKey} -| ${row ? `${row.id}.${row.versionKey}` : 'no patient exist'}`);
  }

  infoNotThisTime(data: { id: number; versionKey: number }, row?: any): void {
    eventLogger.info(`[EVENT] Patient d [NO_ACK  ] ${data.id}.${data.versionKey} -| ${row ? `${row.id}.${row.versionKey}` : 'no exist'}`);
  }
}
