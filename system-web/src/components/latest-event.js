  ...
  useEffect(() => {
    const id = setInterval(() => {
      if (socketContext.latestEvent) {
        const seconds = (
          (new Date().getTime() -
            new Date(socketContext.latestEvent.timestamp).getTime()) /
          1000
        ).toFixed(0);
  ...
  return (
    <>
      <Row align="bottom">
        <Col span={12}>
          <Space align="baseline">
            ...
            {timeSinceLastEvent && (
              <Tag color="green">{timeSinceLastEvent} seconds old</Tag>
            )}
          </Space>
        </Col>
      ...
      <Table
        ...
        dataSource={
          socketContext.latestEvent ? [socketContext.latestEvent] : []
        }
        pagination={{
